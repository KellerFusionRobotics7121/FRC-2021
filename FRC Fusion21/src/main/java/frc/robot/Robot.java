...
/**
 * Description:
 * The DriveStraight_AuxIntegratedSensor example demonstrates the new Talon/Victor auxiliary and 
 * remote features used to perform complex closed loops. This example has the robot driving in 
 * Percent Output with an auxiliary closed loop on integrated sensor difference to keep the robot straight.
 * 
 * This example uses:
 * - 2x Falcon 500 motors with integrated sensors for Auxiliary Closed Loop on Difference (heading)
 * Talon caclulates the heading by taking the difference between both sensors.
 *
...
public class Robot extends TimedRobot {
	...
	TalonFX topShooter = new TalonFX(2);
	TalonFX bottomShooter = new TalonFX(3);
	...
	/* Code for Multiplatform */
	public int returnButtonID(Dictionary dict, String str){
		return dict.get(str);
	}
	Dictionary testMultiPlatformButton = new Hashtable();
	...
	public void teleopInit(){
		...
		/*Ok so here you add your values to the button prompts established above*/
		testMultiPlatformButton.Put("Ethan",5);
		testMultiPlatformButton.Put("Austin",7);
	}
	...
	public void teleopPeriodic() {
		...
		boolean btn6 = _gamepad.getRawButton(6);	//shooter
		boolean btn5 = _gamepad.getRawButton(5);	//conveyor
		boolean btn7 = _gamepad.getRawButton(7);
		boolean btn8 = _gamepad.getRawButton(8);  //intake
...
rightWrist.set(ControlMode.PercentOutput,intakeValue);
leftWrist.set(ControlMode.PercentOutput,-intakeValue);
